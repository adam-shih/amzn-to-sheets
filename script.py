...
class AmazonScript:
    def __init__(self):
        google_sheets = GoogleSheets(
            "gsheets-keys.json", const.GOOGLE_SHEETS_ID, const.GOOGLE_WORKSHEET_NAME
        )
    ...
    def get_orders_from_sp_api(self) -> List[AmazonOrder]:
        client_config = dict(
            refresh_token=const.SP_API_REFRESH_TOKEN,
            lwa_app_id=const.LWA_APP_ID,
            lwa_client_secret=const.LWA_CLIENT_SECRET,
            aws_secret_key=const.SP_API_SECRET_KEY,
            aws_access_key=const.SP_API_ACCESS_KEY,
            role_arn=const.SP_API_ROLE_ARN,
        )
        res = Orders(credentials=client_config, marketplace=Marketplaces.US)
        return self.convert_response_to_amazon_order_list(
            res.get_orders(CreatedAfter='2023-01-01', CreatedBefore=date.today().isoformat()).payload
        )
